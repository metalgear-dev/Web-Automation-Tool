...
class selenium_driver():
    ...
    def create_driver(self):
        ...
        if self.driver_path == "":
            ...
            self.print_log(
                "your driver exe path is {0}".format(
                    self.driver_path))
        ...
        if cur_config['browser_name'] == "chrome":
            ...
            if cur_config['used_proxy']:
                self.print_log(
                    "current proxy ip is : " + cur_config['cur_proxy']
                )
                chrome_options.add_argument(
                    '--proxy-server=' + cur_config['cur_proxy'])
            else:
        ...
        if cur_config['window_pos'] != [0, 0]:
            driver.set_window_position(
                cur_config['window_pos'][0],
                cur_config['window_pos'][1])
        return driver

    # open url in broser, delay time is the limit time for the broswer will
    # wait for responses
    def open_url(self, cur_url, delay_time=60):
        if self.driver:
            ...
            try:
                ...
                self.print_log(
                    "Too much time taken to open {0}".format(cur_url))
                return False
    ...
    def print_log(self, print_str):
        print(
            "{0} says : {1}".format(
                self.options._config['driver_name'],
                print_str))
    ...
    def wait_tag(
        ...
        while True:
            ...
            try:
                ...
                if clickable:
                    if by_type == "css":
                        return_tag = WebDriverWait(
                            self.driver, wait_time).until(
                            EC.element_to_be_clickable(
                                (By.CSS_SELECTOR, by_string)))
                    else:
                        return_tag = WebDriverWait(
                            self.driver, wait_time).until(
                            EC.element_to_be_clickable(
    ...
    def click_element(self, cur_element):
        browser_navigation_panel_height = self.driver.execute_script(
        ...
        pyautogui.click(center_x + randrange(-offset_x, offset_x),
                        center_y + randrange(-offset_y, offset_y))
